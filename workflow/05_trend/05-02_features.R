...
# Data paths --------------------------------------------------------------
# HYDE files
hyde_folders <- # list.dirs("C:/Users/atsyp/Downloads/HYDE/") %>%
...
# Buchner et al., 2020
forest_file <- "E:/WORK/00_GLOBAL/Caucasus landuse/Caucasus_forest_change_map_1987-2015.tif"

# Cropland change in 1987-2015
# Buchner et al., 2020
cropland_file <- "E:/WORK/00_GLOBAL/Caucasus landuse/Caucasus_cropland_change_map_1987-2015.tif"

# Abandond areas
aal_path <- "E:/WORK/00_GLOBAL/hybrid_map_of_abandoned_arable_land_in_fSU/hybrid map of abandoned arable land.tif"

# WorldClim 2.1
wc_parent_path <- "E:/WORK/00_GLOBAL/WorldClim/histroical/5min/"

tmax_files <- fs::dir_ls(paste0(wc_parent_path, "/tmax"))
tmin_files <- fs::dir_ls(paste0(wc_parent_path, "/tmin"))
prec_files <- fs::dir_ls(paste0(wc_parent_path, "/prec"))
...
tmax_mean <- exact_extract(
  tmax_rast,
  st_transform(ws_sf, 4326),
  "mean"
)
tmin_mean <- exact_extract(
  tmin_rast,
  st_transform(ws_sf, 4326),
  "mean"
)
prec_mean <- exact_extract(
  prec_rast,
  st_transform(ws_sf, 4326),
  "mean"
)
prec_cv <- exact_extract(
  prec_rast,
  st_transform(ws_sf, 4326),
  "coefficient_of_variation"
)
prec_med <- exact_extract(
  prec_rast,
  st_transform(ws_sf, 4326),
  "median"
)
...
aal_rast <- rast(aal_path)

aal_freqs <- exact_extract(
  aal_rast,
  st_transform(ws_sf, 4326),
...
# Forest ------------------------------------------------------------------
# Read forest raster
forest_rast <- rast(forest_file)

# Crop to AOI
# Mind the CRS!!!
forest_rast_crop <- crop(
  forest_rast,
...
forest_freqs_tidy <- forest_freqs |>
...
forest_tidy <- forest_freqs_tidy |>
  ...
  mutate(
    f1995 = map2_dbl(
      data,
      f1987,
      ~(.y + .x$area[.x$type == "gain 1995"] - .x$area[.x$type == "loss 1995"])
    )
  ) %>%
  mutate(
    f2000 = map2_dbl(
      data,
      f1995,
      ~(.y + .x$area[.x$type == "gain 2000"] - .x$area[.x$type == "loss 2000"])
    )
  ) %>%
  mutate(
    f2005 = map2_dbl(
      data,
      f2000,
      ~(.y + .x$area[.x$type == "gain 2005"] - .x$area[.x$type == "loss 2005"])
    )
  ) %>%
  mutate(
    f2010 = map2_dbl(
      data,
      f2005,
      ~(.y + .x$area[.x$type == "gain 2010"] - .x$area[.x$type == "loss 2010"])
    )
  ) %>%
  mutate(
    f2015 = map2_dbl(
      data,
      f2010,
      ~(.y + .x$area[.x$type == "gain 2015"] - .x$area[.x$type == "loss 2015"])
    )
  ) %>%
  dplyr::select(-data) %>%
  ungroup() |>
...
cropland_rast <- rast(cropland_file)

# Crop to AOI
# Mind the CRS!!!
cropland_rast_crop <- crop(
  cropland_rast,
...
# Tidy up forest extraction
cropland_freqs_tidy <- cropland_freqs |>
  mutate(ID = ifelse(is.na(value), TRUE, FALSE)) |>
  mutate(IDr = data.table::rleid(ID)) |>
  mutate(IDr = ifelse(ID, lag(IDr), IDr)) |>
  mutate(IDr = data.table::rleid(IDr)) |>
...
# Get forest areas for every year
cropland_tidy <- cropland_freqs_tidy |>
...
get_hyde <- function(
  ...
  full_path <- paste0(.path, our_files)

  # Metadata of the HYDE files
  # YEAR
  year <- str_extract(our_files[1], "\\d{4,4}") %>%
    as.integer()

  # TYPES of the HYDE values
  type <- str_extract(
    our_files,
    "cropland|pasture|grazing|rangeland|conv_rangeland|ir_rice|ir_norice
                 tot_irri|tot_rice|tot_rainfed|rf_rice|rf_norice"
  )

  # Load HYDE rasters
  hyde_rast <- rast(full_path)
...
# approx. 2min
hyde_db <- pblapply(hyde_folders[3:32], get_hyde)

hyde_df <- collapse::unlist2d(hyde_db, idcols = FALSE)
...
glims_path <- "data/vector/glims.gpkg"

glims_sf <- st_read(glims_path) |>
  st_zm() |>
...
get_glims_year <- function(.yr) {
  glims_year <- subset(glims_tidy, glims_tidy$year2 == .yr) |>
    as_geos_geometry()

  glims_area <- vector(mode = "numeric", length = 8L)

  for (i in seq_along(ws_geos)) {
    glims_area[i] <- glims_year[
      geos_prepared_intersects(glims_year, ws_geos[i])
    ] |>
      geos_area() |>
      sum()
  }
