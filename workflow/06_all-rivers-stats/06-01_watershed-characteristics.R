...
cropland_tidy <- 
  ...
  group_by(id) %>% 
  nest() %>% 
  mutate(c1987 = map_dbl(data,
                         ~(.x$area[.x$type == "stable cropland"] +
                             .x$area[.x$type == "transitional cropland"]))) %>% 
  mutate(c1995 = map2_dbl(data, c1987,
                          ~(.y + .x$area[.x$type == "gain 1995"] - 
                              .x$area[.x$type == "loss 1995"]))) %>%   
  mutate(c2000 = map2_dbl(data, c1995,
                          ~(.y + .x$area[.x$type == "gain 2000"] - 
                              .x$area[.x$type == "loss 2000"]))) %>% 
  mutate(c2005 = map2_dbl(data, c2000,
                          ~(.y + .x$area[.x$type == "gain 2005"] - 
                              .x$area[.x$type == "loss 2005"]))) %>% 
  mutate(c2010 = map2_dbl(data, c2005,
                          ~(.y + .x$area[.x$type == "gain 2010"] - 
                              .x$area[.x$type == "loss 2010"]))) %>% 
  mutate(c2015 = map2_dbl(data, c2010,
                          ~(.y + .x$area[.x$type == "gain 2015"] - 
                              .x$area[.x$type == "loss 2015"]))) %>% 
  dplyr::select(-data) %>% 
  ungroup() |> 
...
buchner_areas |> 
  ...
  pull(ForestArea) |>
  chop(c(0.5), labels = lbl_dash()) |> table()

glims_df |> 
  mutate(GlimsArea = GlimsArea/10^6/Area) |> 
  pull(GlimsArea) |> quantile()
...
good_df <- 
  ...
  mutate(
    ...
    AbanGroup = santoku::chop(AAL, c(0, 0.07, 1), left = T,  labels = lbl_dash())
  ) 
...
good_df |> 
  ...
  mutate(across(c( TotAban, CroplandArea), ~{100*.x})) |> 
