...
global_path <- ifelse(
  ...
  "/mnt/e/WORK/00_GLOBAL/",
  "E:/WORK/00_GLOBAL/"
)

# WorldClim 2.1
wc_parent_path <- paste0(global_path, "WorldClim/histroical/5min/")
...
forest_file <- paste0(
  global_path,
  "Caucasus landuse/Caucasus_forest_change_map_1987-2015.tif"
)

# Cropland change in 1987-2015
# Buchner et al., 2020
cropland_file <- paste0(
  global_path,
  "Caucasus landuse/Caucasus_cropland_change_map_1987-2015.tif"
)

# Abandond areas
aal_path <- paste0(
  global_path,
  "hybrid_map_of_abandoned_arable_land_in_fSU/hybrid map of abandoned arable land.tif"
)
...
glims_sf <- st_read(glims_path) |>
  ...
  transmute(
    glac_id,
    year = as.POSIXct(src_date, tz = "UTC") |>
      year()
  ) |>
...
# Emmeans -----------------------------------------------------------------
landcover_complete <- buchner_areas |>
  group_by(id) |>
  complete(Year = seq(1960, 2021, by = 1)) |>
...
m <- lm(ssy ~ prec * CroplandArea * TotAban, data = good_df) # lm(ssy ~ prec + prec*CroplandArea*ForestArea, data = good_df)
broom::glance(m)
broom::tidy(m)
...
emmeans(
  m,
  pairwise ~ CroplandArea | TotAban + prec,
  # cov.reduce = range,
  at = list(
...
emmeans_model_plot <- em_ci$emmeans |>
  as_tibble() |>
  mutate(
...
m_scaled <- good_df |>
  mutate(
    across(
      c(ssy, prec, CroplandArea, TotAban, ForestArea),
      ~{
