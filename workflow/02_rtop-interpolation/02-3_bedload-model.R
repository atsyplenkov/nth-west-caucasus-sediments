...
turowski_all <- read_excel(
  "data/tables/Turowski-etal-2010_table-5.xlsx",
  sheet = "All Data"
)

# 2) Fit a hyperbolic model -----------------------------------------------
hyperbolic_model <- nls(
...
plot(
  turowski_all$mean_sus_load_kgsec,
...
turowski_all$.predict <- predict(hyperbolic_model)
