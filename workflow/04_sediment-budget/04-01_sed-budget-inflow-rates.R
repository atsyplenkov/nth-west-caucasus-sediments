...
plot_sediments <- 
  ...
  scale_color_manual(
    name = "",
    values = clrs,
    labels = c("Kuban (No. 83174)", "Laba (No. 83314)", "Belaya (No. 83361)",
...
rivers_flux <- 
  ...
  unnest(cols = c(data)) |> 
  mutate(
    across(ends_with("Pct"), ~ atslib::smart_round(100 * .x)),
    across(ends_with("Flux"), ~atslib::smart_round(.x))
  ) |> 
  transmute(
    Period, id,
...
plot_krasn_budget <- 
  krasn_budget |> 
  select(Period, V, Shore, VolSS, VolBS) |> 
  gather(
    part, role, -Period, -V
  ) |> 
  group_by(Period) |> 
