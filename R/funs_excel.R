...
xtract_table <- 
  function(){
    ...
    tt %>% 
      ...
      mutate_at(vars(2:13), ~parse_number(.,
      ...
      mutate(data = map2(.x = data, .y = n, ~(if(.y == 1) tibble(month = c(1:12), ss = rep(NA_real_, 12)) else .x))) %>% 
      unnest(data) %>% 
      ungroup() %>% 
      dplyr::select(-n) %>% 
      clipr::write_clip()
...
decade_to_date <- 
  function(year, month, decade){
    ...
    decade_start_date <- 
      split(d, ceiling(seq_along(d)/10)) %>%
...
explore_miss <- 
  function(.data, .var, .year = year, .id = id){
    ...
    .data %>%
      ...
      scale_x_continuous(limits = c(min_max_yr[1], 2022),
                         breaks = seq(min_max_yr[1], 2022,
                                      by = 5)) +
      labs(
        x = "",
        y = "Gage id"
...
read_1976 <- 
  function(.path, .sheet){
    ...
    data_mean <- 
      ...
      mutate(id = as.numeric(station_id),
             .before = year) %>% 
      group_by(id, year) %>% 
      summarise(
